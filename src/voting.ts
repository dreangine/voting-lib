...
export async function validateRegisterVoting(votingParams: VotingParamsValidate): Promise<void> {
  ...
  const allVoters = [startedBy]
  if ('candidates' in votingParams) {
    const { candidates } = votingParams
    ...
    if (candidatesIds.includes(startedBy))
      throw new Error('Voting cannot be started by a candidate')
    allVoters.push(...candidatesIds)
  }
  const checkedVoters = await checkActiveVoters(allVoters)
  const notFoundVoterIds = allVoters.filter((voter) => !checkedVoters[voter])
